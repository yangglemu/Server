...
namespace Server
{
    public partial class AddXL : Form
    {
        int bh;
        ...
        protected void AddXL_Load(object sender, EventArgs e)
        {
            ...
            command.CommandText = "select bh,pm,dnm from fl where char_length(dnm)=4 order by dnm asc";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Error,AddXL.cs 54 line.");
                return;
            }
            MessageBox.Show("添加小类成功！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            command.CommandText = string.Format("select ifnull(max(bh),0)+1 from fl where char_length(dnm)=6 and substring(dnm,1,4)='{0}'",
                g.dnm);
            bh = int.Parse(command.ExecuteScalar().ToString());
            this.textBox1中类.Text = g.dnm + bh.ToString("00");
            this.textBox2品名.Text = g.pm.Substring(0, 2);
            this.textBox2品名.Select();
            this.textBox2品名.SelectionStart = this.textBox2品名.TextLength;
        }

        private void AddXL_KeyDown(object sender, KeyEventArgs e)
        {
